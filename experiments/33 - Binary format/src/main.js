...
class App {
  ...
  constructor() {
    ...
    this.loader.loadManifest([
      // {src: 'keyframes.json'},
      {src: 'keyframes.bin', type: createjs.AbstractLoader.BINARY},
      {src: 'keyframeinfo.json'},
      {id: 'image', src: 'slice_face_28.jpg'},
      {id: 'data', src: 'slice_face_28.json'},
    ])
    this.loader.on('complete', () => {
      console.timeEnd('load assets')
      // let original = this.loader.getResult('keyframes.json')
      // console.log(original)
      let buffer = this.loader.getResult('keyframes.bin')
      console.log(buffer.byteLength)
      let info = this.loader.getResult('keyframeinfo.json')
      console.log(info)
      // console.log(original.camera.property.quaternion)
      console.time('convert')
      let toBeProcessed = [info]
      while (toBeProcessed.length) {
        let obj = toBeProcessed.pop()
        _.forEach(obj, (v, k) => {
          // console.log(k, v, v && v['min'])
      ...
      // console.log(info.camera.property)
      // console.log(this.prepareDataFor(info.camera.property.quaternion, buffer))
  ...
  initObjects() {
    ...
    this.currentFrameGui.setValue(0)
  ...
  set currentFrame(value) {
    ...
    props = this.keyframes.user.property
    ...
    this.face.geometry.applyMorph(props.morph[f])
